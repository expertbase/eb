...
if( $('.clap_fixed_right').length)
{
	...
	function upClickCounter()
	{
		...
		if(accCounter<11)
		{
			clickCounter.children[0].innerText='+'+accCounter;totalClickCounter.innerText=totalCount+accCounter;
		...
		totalClickCounter.innerText="Nice!";return false;
		}
	}
...
$('body').on('click', 'a[href="#comment"]', function() {
	...
	profileid= $("#ebp").data("pid"); aid=$("#ebp").data("aid");
	$("#more").empty().load("article_comment",{id:profileid,aid:aid}, function(){
	  	autosize($('#wpcomments'));new $.Zebra_Tooltips($('.tips'));
	}); 
});
